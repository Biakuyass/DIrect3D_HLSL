...
HRESULT CALLBACK OnD3D9CreateDevice(LPDIRECT3DDEVICE9 pd3dDevice,const D3DSURFACE_DESC *pBackBufferSurfacfeDesc,void *pUserText)
{
	...
	//物体创建设备
	g_renderObject.OnD3D9CreateDevice(pd3dDevice,L"Model.X",L"Project.fx",L"ColorMap.bmp");
...
//	D3DXMatrixRotationY(&g_matWorld,fTime/10.0f);
	...
	//投影纹理的参数设定
	D3DXMATRIX matViewProject,matProjProject;
	D3DXVECTOR3 vEye(10,20,10);
	D3DXVECTOR3 vLookat(0,0,0);
	D3DXVECTOR3 vUp(0,1.0f,0);
	D3DXMatrixLookAtLH(&matViewProject,&vEye,&vLookat,&vUp);

	D3DXMatrixPerspectiveFovLH(&matProjProject,0.5,1.0f,1.0,100);



	g_renderObject.OnFrameMove(fTime,fElapsedTime,&mWorld,&matWorldViewProj);

	D3DXMATRIX matWorldViewProjProject;
	matWorldViewProjProject=mWorld*matViewProject*matProjProject;

	g_renderObject.SetProject(&mWorld,&matWorldViewProjProject);
...
{
	// 取系统默认的格式
	///D3DFORMAT d3dFormat = pDeviceSettings->d3d9.pp.AutoDepthStencilFormat ;
	// 修改成需要的格式
	//用模板缓冲需要更改格式
	pDeviceSettings->d3d9.pp.AutoDepthStencilFormat=D3DFMT_D24S8;//pp.AutoDepthStencilFormat = D3DFMT_D24S8 ;
	return true;
}
