...
void CRenderObject::SetLightFactor(D3DXMATRIX * matWorldInverseTrans,D3DXVECTOR4 *vEyePosition,D3DXVECTOR4*vLightPosition)
{
	HRESULT hr;
	V(m_Effect->SetMatrix("matWorldInverseTrans",matWorldInverseTrans));
	V(m_Effect->SetVector("vEyePosition",vEyePosition));
	V(m_Effect->SetVector("vLightPosition",vLightPosition));
}
...
void CRenderObject::RenderShadow(LPDIRECT3DDEVICE9 pd3dDevice, double fTime, float fElapsedTime, void* pUserContext)
{
	HRESULT hr;
	V(m_Effect->SetTechnique("Shadow"));
	pd3dDevice->SetVertexDeclaration(m_pDecl);
