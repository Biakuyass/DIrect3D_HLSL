...
HRESULT  CRenderObject::OnD3D9ResetDevice(LPDIRECT3DDEVICE9 pd3dDevice,const D3DSURFACE_DESC *pBackBufferSurfaceDesc,void *pUserText)
{
	...
	if( FAILED( hr = pd3dDevice->CreateTexture( 2048 , 2048,1 ,
		D3DUSAGE_RENDERTARGET, D3DFMT_G32R32F , D3DPOOL_DEFAULT , &m_pRTTTextureDepth , NULL ))) return hr;
...
void CRenderObject::RenderToDepthTexture(LPDIRECT3DDEVICE9 pd3dDevice, double fTime, float fElapsedTime, void* pUserContext)
{
	HRESULT hr;
	V(m_Effect->SetTechnique("Depth"));

	if(SUCCEEDED(m_pRTTInterfaceDepth->BeginScene(m_pRTTSurfaceDepth,NULL)))
	{
...
void CRenderObject::OnD3D9DestroyDevice( void* pUserContext )
{
	...
	SAFE_RELEASE(m_pRTTInterfaceDepth);  // 控制纹理的接口
	SAFE_RELEASE(m_pRTTSurfaceDepth);    // 页面
