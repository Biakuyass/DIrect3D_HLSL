...
D3DXMATRIX             g_matLight;

D3DXVECTOR4            g_LightPosition(12,13,7,1);
...
HRESULT CALLBACK OnD3D9CreateDevice(LPDIRECT3DDEVICE9 pd3dDevice,const D3DSURFACE_DESC *pBackBufferSurfacfeDesc,void *pUserText)
{
	...
	//物体创建设备
	g_renderObject.OnD3D9CreateDevice(pd3dDevice,L"Model.X",L"VSM.fx",L"ColorMap.bmp");
	...
	D3DXMatrixTranslation(&g_matLight,g_LightPosition.x,g_LightPosition.y,g_LightPosition.z);
...
//	D3DXMatrixRotationY(&g_matWorld,fTime/10.0f);
	...
	//从光源的取景变换
	D3DXMATRIX matRTTView;
	D3DXMATRIX matRTTProj;

	D3DXVECTOR3 vEyept(15*sin(fTime/2.0),16,15*cos(fTime/2.0));
	D3DXVECTOR3 vUP(0,1.0,0);
	D3DXVECTOR3 vLookAt(0,-8,0);

	D3DXMatrixLookAtLH(&matRTTView,&vEyept,&vLookAt,&vUP);
	D3DXMatrixPerspectiveFovLH(&matRTTProj,D3DX_PI/2.0,1.0f,1.0f,100);


	D3DXMATRIX matRTTWorldViewProj=g_matWorld*matRTTView*matRTTProj;
	g_renderObject.SetSpecialMatrix(&matRTTWorldViewProj);
}
...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	g_renderObject.RenderToDepthTexture(pd3dDevice,fTime,fElapsedTime,pUserContext);

	g_renderObject.SetDepthTexture(g_renderObject.GetDepthTexture());
	HRESULT hr;
	...
	if( SUCCEEDED( pd3dDevice->BeginScene() ) )
	{
       ...
       g_Cboard.SetTexture(g_renderObject.GetDepthTexture());
