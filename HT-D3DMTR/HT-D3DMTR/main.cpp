...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if( SUCCEEDED( pd3dDevice->BeginScene() ) )
	{
		...
		g_Cboard.SetRenderTexture(g_renderObject.ReturnPositionTexture(),1);
		g_Cboard.OnD3D9FrameRender(pd3dDevice,fTime,fElapsedTime,pUserContext);

		g_Cboard.SetRenderTexture(g_renderObject.ReturnNormalTexture(),2);
		g_Cboard.OnD3D9FrameRender(pd3dDevice,fTime,fElapsedTime,pUserContext);

		g_Cboard.SetRenderTexture(g_renderObject.ReturnDepthTexture(),3);
		g_Cboard.OnD3D9FrameRender(pd3dDevice,fTime,fElapsedTime,pUserContext);
