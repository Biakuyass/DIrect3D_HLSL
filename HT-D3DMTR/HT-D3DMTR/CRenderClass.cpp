...
HRESULT  CRenderObject::OnD3D9ResetDevice(LPDIRECT3DDEVICE9 pd3dDevice,const D3DSURFACE_DESC *pBackBufferSurfaceDesc,void *pUserText)
{
	...
	HRESULT hr;
	// ------------------- 位置纹理 -----------------------------
	if( FAILED( hr = pd3dDevice->CreateTexture( pBackBufferSurfaceDesc->Width , pBackBufferSurfaceDesc->Height , 1 ,
		D3DUSAGE_RENDERTARGET, D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &m_pRTTTexturePosition , NULL ))) return hr;
	// ------------------- 法线纹理 -----------------------------
	if( FAILED( hr = pd3dDevice->CreateTexture( pBackBufferSurfaceDesc->Width , pBackBufferSurfaceDesc->Height , 1 ,
		D3DUSAGE_RENDERTARGET, D3DFMT_A16B16G16R16F, D3DPOOL_DEFAULT, &m_pRTTTextureNormal , NULL ))) return hr;
	//--------------------- 深度纹理 -------------------------
	if( FAILED( hr = pd3dDevice->CreateTexture( pBackBufferSurfaceDesc->Width , pBackBufferSurfaceDesc->Height , 1 ,
		D3DUSAGE_RENDERTARGET, D3DFMT_A16B16G16R16F , D3DPOOL_DEFAULT , &m_pRTTTextureDepth , NULL ))) return hr;
...
void CRenderObject::RenderToTexture(LPDIRECT3DDEVICE9 pd3dDevice, double fTime, float fElapsedTime, void* pUserContext)
{
	...
	SAFE_RELEASE( pRenderTargetPosition );
	SAFE_RELEASE( pRenderTargetNormal );
...
void CRenderObject::OnD3D9DestroyDevice( void* pUserContext )
{
	...
	SAFE_RELEASE(m_pRTTTexturePosition);  // 纹理
	SAFE_RELEASE(m_pRTTTextureNormal);    // 纹理
	SAFE_RELEASE(m_pRTTTextureDepth);     // 纹理
