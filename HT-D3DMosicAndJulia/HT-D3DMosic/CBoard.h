...
public:
	...
	void SetFloatForJulia(float fScale_){m_pEffect->SetFloat("fScale",fScale_);}
	void SetMoveX(float MoveX_){m_pEffect->SetFloat("MoveX",MoveX_);}
	void SetMoveY(float MoveY_){m_pEffect->SetFloat("MoveY",MoveY_);}

	void SetTexture(LPDIRECT3DTEXTURE9 pTexture_)
	{
		//m_pTexture=pTexture_;
	    m_pEffect->SetTexture("g_Texture",pTexture_);
	}
