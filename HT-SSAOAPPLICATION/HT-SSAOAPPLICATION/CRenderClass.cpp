...
HRESULT  CRenderObject::OnD3D9ResetDevice(LPDIRECT3DDEVICE9 pd3dDevice,const D3DSURFACE_DESC *pBackBufferSurfaceDesc,void *pUserText)
{
	...
	m_Effect->SetTexture("g_Texture",m_pTexture);
...
void CRenderObject::ReturnToTexture(LPDIRECT3DDEVICE9 pd3dDevice,D3DXMATRIX* pmatWorld,D3DXMATRIX* pmWorldViewProj)
{

	HRESULT hr; 
	// 渲染深度信息到纹理
	hr = m_Effect->SetTechnique( "CommonRender" );
	if( SUCCEEDED( hr = m_pRTTInterfaceDepth->BeginScene( m_pRTTSurfaceDepth , NULL )))
	{
		...
		hr = pd3dDevice->SetStreamSource( 0, m_pVertexBuffer, 0, sizeof(VS_INPUT )); 
		hr = pd3dDevice->SetIndices( m_pIndexBuffer ); 

		UINT uPasses;
		hr = m_Effect->Begin( &uPasses, D3DXFX_DONOTSAVESTATE );
