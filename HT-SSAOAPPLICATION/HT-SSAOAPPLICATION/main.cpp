...
CRenderObject          g_RenderIObj;
CRenderObject          g_RenderIObj_Real;
CBoard                 g_Cboard;
...
HRESULT CALLBACK OnD3D9CreateDevice(LPDIRECT3DDEVICE9 pd3dDevice,const D3DSURFACE_DESC *pBackBufferSurfacfeDesc,void *pUserText)
{
	...
	g_RenderIObj.OnD3D9CreateDevice(pd3dDevice,L"ResFile_1_Ground\\Model.X",L"RenderToTexture.fx",L"ResFile_1_Ground\\ColorMap.jpg");
	g_RenderIObj_Real.OnD3D9CreateDevice(pd3dDevice,L"ResFile_1_Ground\\Model.X",L"Test.fx",L"ResFile_1_Ground\\ColorMap.jpg");



	D3DXVECTOR3 vecEye(  15  , 15 , 0.0);
...
HRESULT CALLBACK OnD3D9ResetDevice(LPDIRECT3DDEVICE9 pd3dDevice,const D3DSURFACE_DESC *pBackBufferSurfaceDesc,void *pUserText)
{
	...
	g_RenderIObj.OnD3D9ResetDevice(pd3dDevice,pBackBufferSurfaceDesc,pUserText);
	g_RenderIObj_Real.OnD3D9ResetDevice(pd3dDevice,pBackBufferSurfaceDesc,pUserText);
...
void CALLBACK OnFrameMove(double fTime, float fElapsedTime, void* pUserContext )
{
	...
	D3DXMATRIX g_mWorldViewProj=g_matWorld*mView*mProj;


	g_RenderIObj.OnFrameMove(fTime,fElapsedTime,&g_matWorld,&g_mWorldViewProj);
	g_RenderIObj_Real.OnFrameMove(fTime,fElapsedTime,&g_matWorld,&g_mWorldViewProj);


	g_mWorldViewProj=g_matLightPosition*mView*mProj;
...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	g_RenderIObj.ReturnToTexture(pd3dDevice,&g_matWorld,&g_mWorldViewProj);

	LPDIRECT3DTEXTURE9 ptemp_texture=g_RenderIObj.ReturnDepthTexture();
	g_Cboard.SetTexture(ptemp_texture);

	g_Cboard.ReturnToTexture(pd3dDevice,&g_matWorld,&g_mWorldViewProj);

	g_RenderIObj_Real.SetSSAOTexture(g_Cboard.ReturnSSAOTexture());
...
/*	if(UI::RenderOut(pd3dDevice,fTime,fElapsedTime,pUserContext))
	...
	{
	//	g_RenderIObj.OnD3D9FrameRender(pd3dDevice,fTime,fElapsedTime,pUserContext);


		g_RenderIObj_Real.OnD3D9FrameRender(pd3dDevice,fTime,fElapsedTime,pUserContext);

		g_Cboard.OnD3D9FrameRender(pd3dDevice,fTime,fElapsedTime,pUserContext);//使用了颜色混合加上去的
...
void CALLBACK OnD3D9LostDevice( void* pUserContext )
{
	...
	g_RenderIObj.OnD3D9LostDevice(pUserContext);
	g_RenderIObj_Real.OnD3D9LostDevice(pUserContext);
...
void CALLBACK OnD3D9DestroyDevice( void* pUserContext )
{
	...
	g_RenderIObj.OnD3D9DestroyDevice(pUserContext);
	g_RenderIObj_Real.OnD3D9DestroyDevice(pUserContext);
