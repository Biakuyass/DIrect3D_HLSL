...
void  CRenderObject:: OnFrameMove(double fTime, float fElapsedTime,D3DXMATRIX* pmatWorld,D3DXMATRIX* pmWorldViewProj,D3DXVECTOR4 *vEyept,D3DXVECTOR4 *vBillBoardPosition)
{
	HRESULT hr;
	//矩阵设置
//	D3DXMATRIX mWorld=matWorld;
	...
	V(m_Effect->SetFloat("g_fTime",fTime));
	V(m_Effect->SetVector("g_vEye",vEyept));
	V(m_Effect->SetVector("g_vPosition",vBillBoardPosition));
	//V(m_Effect->SetFloat("g_fAlpha",0.5f));
