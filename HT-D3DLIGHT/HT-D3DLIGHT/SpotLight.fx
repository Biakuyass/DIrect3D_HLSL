...
float3 vLightDirection;//聚光灯的正对方向

static float g_fCone=0.9f;
...
//	float4 SpecularMaterial=float4(material._31,material._32,material._33,1.0);
	...
	if(angle>g_fCone)
	{
	DiffusePower= saturate(1*dot(Input.Normal,normalize(Input.vLight)));//漫反射的强度
	SpecularPower=pow(saturate(dot(reflect(normalize(Input.vLight),Input.Normal),Input.vEye)),10);//镜面反射的强度
	}
	else
	{
		DiffusePower=0;
